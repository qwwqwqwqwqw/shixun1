...
| `aiserver_pkg/aiserver_node.py` | **★ TCP Socket 服务节点**：接收小程序指令，发布到 `/command_room`；接收人脸识别结果，发布到 `/face_room`；订阅 `/navigation_status` 回传前端 | D | TCP JSONL 消息 | `/command_room` `(String)`、`/face_room` `(String)` | socket、std_msgs |
