...
│           ├── aiserver_pkg/
│           │   ├── __init__.py
│           │   └── aiserver_node.py     # ★ TCP/WebSocket 服务节点
│           ├── launch/
...
| `aiserver_pkg/aiserver_node.py` | **★ TCP/WebSocket 服务节点**：接收小程序/Web 前端指令，发布到 `/command_room`；接收人脸识别结果，发布到 `/face_room`；订阅 `/navigation_status` 回传前端 | D | 前端 WebSocket 消息 | `/command_room` `(String)`、`/face_room` `(String)` | websockets、std_msgs |
...
| 文件 | 功能描述 | 负责人 | 输入 | 输出 | 依赖 |
|------|----------|--------|------|------|------|
| `face_recognizer.py` | **★ 人脸识别节点**：检测摄像头人脸 → 对比 `known_faces/` → 查 `face_room_map.yaml` 映射房间号 → 发布到 `/face_room` | C | `/camera/image_raw` `(Image)` | `/face_room` `(String)` | face_recognition、`face_room_map.yaml` |
...
| 文件 | 功能描述 | 负责人 | 输入 | 输出 | 依赖 |
|------|----------|--------|------|------|------|
| `miniprogram/app.js` | 小程序入口，初始化 WebSocket 连接到 aiserver_node | D | — | WebSocket 连接 | aiserver_node |
| `miniprogram/app.json` | 小程序全局配置，定义页面路由 | D | — | — | — |
| `miniprogram/app.wxss` | 小程序全局样式 | D | — | — | — |
| `miniprogram/pages/index/*` | 首页：教室列表选择 + 人脸识别触发按钮 | D | 用户点击 | WebSocket 指令 | aiserver_node |
| `miniprogram/pages/status/*` | 状态页：实时显示导航状态和到达确认 | D | WebSocket 消息 | UI 展示 | aiserver_node |
| `web/index.html` | Web 控制台页面（备用方案） | D | 用户输入 | WebSocket 指令 | aiserver_node |
| `web/script.js` | WebSocket 通信脚本 | D | — | WebSocket 连接 | aiserver_node |
...
| 文件/目录 | 角色 | 说明 |
|-----------|------|------|
| `dev_ws/src/aiserver_pkg/aiserver_pkg/aiserver_node.py` | **负责** | TCP/WebSocket 桥接节点 |
...
① app/miniprogram/pages/index/index.js     # 用户点击 "开始导航"
                │ WebSocket ({type: "command_room", data: "101"})
                ▼
② dev_ws/src/aiserver_pkg/aiserver_node.py  # 收到前端消息
                │ 发布话题 /command_room (std_msgs/String, data="101")
                ▼
③ dev_ws/src/guide_pkg/guide_pkg/guide_node.py  # on_command_room() 回调
                │
                ├── 调用 _navigate_to("101", source="manual")
                │
                ├── 发布 /navigation_status: "开始导航到 101（来源: manual）"
                │
                ├── 读取 config/classrooms.yaml → 查找 "101" 坐标 (x, y, yaw)
                │       (A组员提供的坐标值)
                │
...
④ Nav2 开始导航 (AMCL 定位 + 全局/局部路径规划)
                        ...
                        │
                        └── 到达目标后发布 /navigation_status: "到达 101"
                                │
                                ▼
⑤ dev_ws/src/guide_pkg/guide_pkg/arrival_fusion.py
                │
                ├── on_nav_status() "到达 101" → nav_done = True
                │
...
⑥ arrival_fusion 发布 /arrival_confirmed (Bool=True)
                │
                └── aiserver_node → 小程序 status 页显示 "已到达 101 ✓"
```
...
① vision/face/face_recognizer.py  # 检测到人脸
                ...
                ├── 匹配到 zhang_san.jpg
                │
                ├── 读取 config/face_room_map.yaml → zhang_san: "101"
                │
                └── 发布话题 /face_room (std_msgs/String, data="101")
                        │
                        ▼
...
④ 导航到达后，vision/doorplate/doorplate_detector.py
                ...
                ├── YOLOv5 检测门牌区域
                ├── OCR 提取门牌号码 "101"
                └── 发布话题 /doorplate_result (std_msgs/String, data="101")
                        │
                        ▼
